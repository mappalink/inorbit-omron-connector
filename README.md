...
# InOrbit Omron Connector

[![Python 3.12](https://img.shields.io/badge/python-3.12-blue.svg)](https://www.python.org/downloads/)

InOrbit Edge connector for Omron

## Overview

This repository contains the [InOrbit](https://inorbit.ai/) Edge Connector for [Omron](https://www.omron.com/) HD1500 robots. It runs one process per robot and communicates via the ARCL (Advanced Robotics Command Language) TCP protocol.

This integration requires the Connector to be configured following the instructions below.

## API Documentation

This connector implementation is based on the Omron ARCL protocol. For detailed documentation, refer to:

- ARCL Reference Guide (provided by Omron with your Enterprise Manager installation)
...
- Access to an Omron robot with ARCL enabled (TCP port 7171)
- Network connectivity between the connector host and Omron server
...
- Copy `config/fleet.example.yaml` to `config/my_fleet.yaml` and configure your robot fleet. Each robot needs an InOrbit `robot_id` and the corresponding Omron `fleet_robot_id`.

- Optionally, configure the Omron connector-specific settings via environment variables. Copy `config/example.env` to `config/.env` and fill in the values. Any `connector_config` fields can be set using the `INORBIT_OMRON_` prefix (e.g., `INORBIT_OMRON_FLEET_HOST`, `INORBIT_OMRON_FLEET_USERNAME`). Environment variables are used as fallbacks when fields are missing from the YAML configuration. See `config/example.env` for reference. The `.env` file will be automatically loaded when the connector is run.
...
```bash
source config/.env && inorbit-omron-connector -c config/my_fleet.yaml
```
...
- **Issues**: [GitHub Issues](https://github.com/mappalink/inorbit-omron-connector/issues)
